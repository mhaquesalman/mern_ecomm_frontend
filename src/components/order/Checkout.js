...
const Checkout = ({ history }) => {
    ...
    const [totalPrice, setTotalPrice] = useState(history.location.state.data.totalPrice)
    const [couponUsed, setCouponUsed] = useState(history.location.state.data.couponUsed)
                    ...
                    <div className="col-md-5">
                        <div className="card" style={{ height: 'auto' }}>
                            ...
                            <div className="card-footer">
                                <span className="float-left"><b>Order Total {couponUsed ? "(After Discount)" : ""}</b></span>
                                <span className="float-right"><b>৳ {totalPrice}</b></span>
                            </div>
                        </div>
                        <br />
                        <p><Link className="btn btn-warning btn-md" to= {{ pathname: "/payment", 
                                state: { data : { totalPrice: totalPrice } } }}>Make Payment</Link></p>
                    </div>
                </div>
            </div>
        </Layout>
